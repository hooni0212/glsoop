(() => {
  ...
  function renderListItems(lists, selectedId = null) {
    ...
    if (!lists.length) {
      ...
      postsEl.innerHTML =
        '<div class="bookmark-empty-state"><span class="emoji" aria-hidden="true">📂</span><p class="gls-mb-1 fw-semibold">폴더를 만든 뒤 글을 저장해 보세요.</p><p class="gls-text-muted gls-text-small gls-gls-mb-0">피드에서 마음에 드는 글을 북마크하면 여기서 모아볼 수 있습니다.</p></div>';
  ...
  function renderEmptyState() {
    postsEl.innerHTML = `
      <div class="bookmark-empty-state">
        ...
        <p class="gls-text-muted gls-text-small gls-gls-mb-0">피드에서 마음에 드는 글을 북마크해 보세요.</p>
      </div>
    `;
  }
})();

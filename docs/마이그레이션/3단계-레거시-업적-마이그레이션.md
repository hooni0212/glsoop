...
### 발견된 레거시 정의/룰
- **업적 카탈로그 데이터**: `docs/마이그레이션/레거시-업적-카탈로그.json`에 레거시 업적 목록(코드/타이틀/조건/아이콘)이 JSON으로 남아 있습니다.
...
### 4단계 정리 (완료)
- `migrations/0007_drop_legacy_achievements_tables.sql`로 레거시 테이블(`achievements`, `user_achievements`)을 제거했습니다.
- `db.js` 자동 초기화 로직에서도 레거시 업적 테이블 생성 코드를 제거했습니다.
- `scripts/verify_phase4.sh`로 레거시 테이블 제거 여부를 검증할 수 있습니다.
